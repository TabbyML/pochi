...
export const MonitorEventsPart: React.FC<{
  ...
  const groups = new Map<string, MonitorEventBatch[]>();
  for (const batch of batches) {
    const group = groups.get(batch.backgroundJobId) ?? [];
    group.push(batch);
    groups.set(batch.backgroundJobId, group);
  }
  return (
    <>
      {Array.from(groups, ([id, group]) => (
        <MonitorEventGroup key={id} batches={group} />
      ))}
    </>
  );
};

const MonitorEventGroup: React.FC<{ batches: MonitorEventBatch[] }> = ({
  batches,
}) => {
