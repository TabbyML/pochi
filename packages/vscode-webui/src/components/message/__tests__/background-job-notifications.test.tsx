...
import { BackgroundJobNotifications, MessageNotifications, } from "../background-job-notifications";
...
vi.mock("@/components/ui/collapsible-section", () => ({
    CollapsibleSection: ({ title, actions, children, }: {
...
vi.mock("@/features/tools", () => ({
    BackgroundJobPanel: ({ backgroundJobId, command, summary, status, outputFile, appearance, }: {
        ...
        outputFile?: string;
        appearance?: string;
    }) => (<div data-testid="background-job-panel" data-background-job-id={backgroundJobId} data-status={status} data-output-file={outputFile} data-appearance={appearance}>
      <span>{command}</span>
      <span>{summary}</span>
    </div>),
}));
vi.mock("../subagent-results", () => ({
    SubagentResultNotificationItem: ({ result, }: {
        result: {
            taskId: string;
            title: string;
        };
...
describe("BackgroundJobNotifications", () => {
    it("groups mixed parts in order, counting individual subagent results", () => {
        const { container, getByText } = render(<MessageNotifications parts={[
        ...
        expect([...container.querySelectorAll("[data-testid]")].map((node) => node.getAttribute("data-task-id") ??
            node.getAttribute("data-background-job-id"))).toEqual(["bgjob-cmd-1", "child-1", "child-2", "bgjob-cmd-2", "child-3"]);
        expect(container.textContent).not.toContain("User message stays outside notifications");
    });
    it("renders no group for messages without notifications", () => {
        const { container } = render(<MessageNotifications parts={[
                { type: "text", text: "Hello" }
            ]}/>);
        expect(container.innerHTML).toBe("");
    });
    it("groups notifications under one data-style section", () => {
        const { container, getAllByTestId, getByText } = render(<BackgroundJobNotifications notifications={[
        ...
        expect(getAllByTestId("background-job-panel")[0]?.getAttribute("data-appearance")).toBe("notification");
...
function notification(backgroundJobId: string, status: BackgroundJobNotification["status"]): BackgroundJobNotification {
    return {
        kind: "command",
