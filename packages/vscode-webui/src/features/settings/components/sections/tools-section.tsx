...
const ToolDescriptions: Record<Exclude<ToolName, "createReview">, string> = {
  ...
  readBackgroundJobOutput:
    "After starting a background job, Pochi uses the readBackgroundJobOutput tool to poll its progress and retrieve new output or logs. It can also catch up on output from a user-opened terminal. This is useful when Pochi needs one eventual result from a build, test, or other long-running command.\n\nPochi can use this tool periodically to check a process or gather information needed for subsequent steps. Each read returns only content produced since the previous check.",
  renderWidget:
