...
const ToolDescriptions: Record<Exclude<ToolName, "createReview">, string> = {
  ...
  startMonitor:
    "The startMonitor tool lets Pochi watch something in the background and react when it changes, without pausing the conversation. Pochi runs a command whose output lines become events - for example tailing a log file for errors, polling a CI job for status changes, or watching a directory for file changes.\n\nUnlike a regular background job that Pochi has to poll, monitor events are delivered to Pochi proactively: when an event arrives, it is queued and handed to Pochi between steps, so Pochi can keep working and interject when something happens. The monitor runs in a visible terminal like any background job, and can be stopped at any time with killBackgroundJob or by closing its terminal.",
  newTask:
