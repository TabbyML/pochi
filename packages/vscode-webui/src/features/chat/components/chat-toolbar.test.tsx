...
import type { BackgroundNotificationPart, Message, Task, } from "@getpochi/livekit";
...
const chatSubmitMocks = vi.hoisted(() => {
    ...
    const setQueuedMessages = {
        current: undefined as React.Dispatch<React.SetStateAction<unknown[]>> | undefined,
    };
    return {
        handleSteerQueuedMessage,
        handleSteerBackgroundJobNotifications,
        setQueuedMessages,
        useChatSubmit: vi.fn((props: {
            setQueuedMessages: unknown;
        }) => {
            setQueuedMessages.current = props.setQueuedMessages as React.Dispatch<React.SetStateAction<unknown[]>>;
            return {
                handleSubmit: vi.fn(),
...
vi.mock("@/components/ui/button", () => ({
    Button: ({ children, ...props }: React.ComponentProps<"button">) => (<button type="button" {...props}>
      {children}
    </button>),
}));
...
vi.mock("@/features/todo", async (importOriginal) => {
    ...
    const TodoList = Object.assign(({ children }: {
        children: React.ReactNode;
...
function renderToolbar(isSubTask: boolean, lastCheckpointHash?: string, { messages = [], flushBackgroundJobNotifications, pendingBackgroundJobNotifications, }: RenderToolbarOptions = {}) {
    render(<ChatToolbar chat={{
            messages,
            sendMessage: vi.fn(),
            addToolOutput: vi.fn(),
            status: "ready",
        } as never} approvalAndRetry={{ pendingApproval: undefined, retry: vi.fn() } as never} compact={vi.fn()} attachmentUpload={{
            files: [],
            isUploading: false,
        ...
        } as never} isSubTask={isSubTask} task={{
            id: "task-1",
            todos: undefined,
            totalTokens: 0,
            lastCheckpointHash,
        } as unknown as Task} displayError={undefined} todos={[auditTodo]} updateTodos={vi.fn()} updateTodoCompletion={vi.fn()} todoPaused={false} onTodoPausedChange={vi.fn()} taskId="task-1" flushBackgroundJobNotifications={flushBackgroundJobNotifications} pendingBackgroundJobNotifications={pendingBackgroundJobNotifications}/>);
}
...
function pendingFollowupQuestionMessages(state: "input-available" | "output-available"): Message[] {
    return [
        {
            id: "message-1",
            role: "assistant",
...
describe("ChatToolbar", () => {
    ...
    it("disables todo creation while active todos exist", () => {
        renderToolbar(false);
        expect(chatSubmitMocks.useChatSubmit).toHaveBeenCalledWith(expect.objectContaining({
            canCreateTodo: false,
        }));
    });
    ...
    describe("background job notification delivery", () => {
        ...
        it("steers the notification entry through the chat kit", async () => {
            ...
            expect(chatSubmitMocks.handleSteerBackgroundJobNotifications).toHaveBeenCalled();
            expect(chatSubmitMocks.handleSteerQueuedMessage).not.toHaveBeenCalled();
        });
    });
});
