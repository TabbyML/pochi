import { type BackgroundJobNotification, createBackgroundJobNotification, } from "@getpochi/common";
import type { ChatInit } from "ai";
import { describe, expect, it, vi } from "vitest";
import type { BlobStore, LiveKitStore, Message } from "../..";
import { createBackgroundJobNotificationMessage, getBackgroundJobNotificationIds, toBackgroundJobNotificationParts, } from "../background-job-notification";
...
describe("LiveChatKit background job notification delivery", () => {
    it("delivers mixed subagent and command notifications with the next user request", async () => {
        ...
        expect(chatKit.chat.messages[1].parts.map((part) => part.type)).toEqual([
            "text", "data-background-job-notification", "data-background-job-notification"
        ]);
    ...
    it("deduplicates task outcomes but delivers a different terminal status", async () => {
        ...
        const retried = { ...subagentResult, notificationId: "bgjob-task-child-1:terminal:failed", status: "failed" as const };
    ...
    it("rides along with the user message that is being sent", async () => {
        ...
        await makeRequest(chatKit);
        expect(chatKit.chat.messages).toHaveLength(2);
        expect(getBackgroundJobNotificationIds(chatKit.chat.messages[1].parts)).toEqual(["bgjob-cmd-1:terminal"]);
        expect(chatKit.pendingBackgroundJobNotifications).toEqual([]);
    });
    ...
    it("ignores notifications that are pending or already delivered", () => {
        ...
        chatKit.chat.messages = [
            userMessage("run it"),
            createBackgroundJobNotificationMessage(toBackgroundJobNotificationParts(notifications("bgjob-cmd-1")))
        ];
        chatKit.enqueueBackgroundJobNotifications(notifications("bgjob-cmd-1"));
        expect(chatKit.pendingBackgroundJobNotifications).toEqual([]);
    ...
    it("starts a turn of its own through the host sender", () => {
        ...
        expect(getBackgroundJobNotificationIds(startTurn.mock.calls[0][0].parts)).toEqual(["bgjob-cmd-1:terminal"]);
    });
...
function makeChatKit(backgroundJobNotifications?: LiveChatKitBackgroundJobNotificationOptions) {
...
function notifications(...backgroundJobIds: string[]): BackgroundJobNotification[] {
    return backgroundJobIds.map((backgroundJobId) => createBackgroundJobNotification({
        taskId: "task-1",
        backgroundJobId,
...
const subagentResult = {
 kind: "subagent" as const, notificationId: "bgjob-task-child-1:terminal:completed", backgroundJobId: "bgjob-task-child-1",
    taskId: "child-1",
