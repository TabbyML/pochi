...
export function createNewCustomAgentMiddleware(
  store: Store,
  parentTaskId: string,
  ...
  return {
    ...
    transformParams: async ({ params }) => {
      ...
      for (const x of tools) {
        if (x.name !== "newCustomAgent") continue;
        if (x.type === "function") {
          ...
          async transform(chunk, controller) {
            ...
            if (
              ...
              const parsedResult = await safeParseJSON({
                text: chunk.input,
                schema: buildClientTools().newCustomAgent.inputSchema,
              });
              ...
              const args = parsedResult.value as InferToolInput<
                ClientTools["newCustomAgent"]
              >;

              const agent = customAgents.find((a) => a.name === args.agentType);
              if (!agent) {
                throw new InvalidToolInputError({
                  toolName: "newCustomAgent",
                  toolInput: chunk.input,
                  cause: new Error(
                    `Agent ${args.agentType} not found in available agents ${customAgents.map((a) => a.name).join(", ")}`,
                  ),
                });
              }
