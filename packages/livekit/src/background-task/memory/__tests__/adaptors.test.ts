...
import { AutoMemoryAdaptor, createAutoMemoryBackendFromManager } from "../auto-memory";
...
describe("auto-memory adaptor", () => {
  it("starts an extraction background task after the main task completes", async () => {
    ...
    const adaptor = new AutoMemoryAdaptor({
      ...
      parentTaskId: "parent",
      parentCwd: "/repo",
      backend: createAutoMemoryBackendFromManager(makeAutoMemoryManager()),
    });
  ...
  it("records direct memory writes without starting extraction", async () => {
    ...
    const adaptor = new AutoMemoryAdaptor({
      ...
      parentTaskId: "parent",
      parentCwd: "/repo",
      backend: createAutoMemoryBackendFromManager(manager),
    });
