...
const getLfsPatterns = async (workspacePath: string) => {
  try {
    ...
    if (await isFileExists(attributesPath)) {
...
/**
 * Returns a comprehensive list of exclude patterns for workspace file listing.
 * Includes build artifacts, media files, cache files, config files, large data files,
 * database files, geospatial files, log files, and Git LFS tracked files.
 */
export const getWorkspaceExcludePatterns = async (
