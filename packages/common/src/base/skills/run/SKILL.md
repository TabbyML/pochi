...
2. Inspect `.pochi/skills/` and `.agents/skills/` for a project-specific skill whose description covers the unit. Invoke the matching skill with `useSkill`; if it is not registered but its `SKILL.md` is present, read and follow it directly.
...
6. Launch with the appropriate Pochi tool:
   ...
   - Use `startBackgroundJob` for servers, watchers, and other long-lived processes.
   - Poll readiness with `readBackgroundJobOutput` or a real health/interface check. Do not rely on a fixed delay when an observable ready condition exists.
...
9. Stop every process or browser session that this task started. Never stop a process that belonged to the user before the task.
