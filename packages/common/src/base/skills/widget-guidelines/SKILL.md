...
- IMPORTANT: Treat `window.pochi.state` as the only UI state source of truth. A `render()` function should read from `window.pochi.state` and derive selected classes, text, inputs, charts, and disabled states from it. Do not keep separate hidden state that can drift from the visible UI.
- `window.pochi.sendMessage(prompt)` is allowed only for widget-authored follow-up messages, such as buttons that ask the model to continue from the selected item. It sends only the prompt string. Call `window.pochi.setState(nextState)` first when the same interaction changes state; do not serialize state into the prompt because the host commits state separately.
