...
- **Built-in Tools for MCP:** Pochi’s MCP got a major upgrade. It now supports server-side tools, allowing Pochi to register and expose capabilities that come bundled directly with the app. The first two built-in tools introduced are `webFetch` and `webSearch`. These let AI agents fetch, read, and process web content directly, without needing external MCP servers. **[#447](https://github.com/TabbyML/pochi/issues/447)**
