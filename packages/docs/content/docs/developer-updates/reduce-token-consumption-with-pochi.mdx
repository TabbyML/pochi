...
- **[Forking a task](https://docs.getpochi.com/developer-updates/#-features-6)** creates a new task from a snapshot of the current workspace and conversation at the moment of the fork. The new task starts from that point with its own independent execution, and the conversation history is clipped to the fork boundary so it doesn’t carry later discussion or exploration. <br></br>

   This makes it useful when you want to try a different approach or explore an alternative implementation without affecting the original task’s trajectory.
<video
        controls
        style={{
        width: "100%",
