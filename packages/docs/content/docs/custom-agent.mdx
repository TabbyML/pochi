...
Rule entries follow the format `tool` or `tool(specifier)`, and tool-specific rules are currently supported for `executeCommand` and `newTask`.
...
Use `newTask(AgentName)` rules to restrict which subagents Pochi can use.

- `newTask(explore)` means the allowed subagent is `explore`.
- `newTask(explore,plan)` means the allowed subagents are `explore` and `plan`.
