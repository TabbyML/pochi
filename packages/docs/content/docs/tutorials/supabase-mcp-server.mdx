...
In this tutorial, you’ll learn how to build a custom MCP server that connects Supabase to Pochi, allowing you to query your database using natural language directly from your code editor.

This is a practical example of how to extend Pochi using the Model Context Protocol (MCP), and illustrates how teams can integrate AI into internal workflows for powerful, context-aware automation. This is particularly useful when you're debugging code, collaborating across teams, or building features that require quick access to live data (e.g: checking whether a migration seeded correctly, inspecting a table schema, or verifying specific column values). 
...
server.tool(
  "queryDatabase",
  {
    inputSchema: z.any(),   // loosened to tolerate Pochi sending strings
    outputSchema: z.any(),  // same here
  },
  async (input) => {
    try {
      let question = "";
...
```bash
pochi -p "What are the total sales for each product by region?"
```
