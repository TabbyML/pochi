---
title: 'NES Series (Part 1): The Edit Model Behind Pochi’s Tab Completion'
---

# NES Series (Part 1): The Edit Model Behind Pochi’s Tab Completion

![NES Cover Image](./images/cover-nes.png)

Today, we’re launching something we’ve been quietly excited about for weeks: Next Edit Suggestion, or simply put, NES. It’s an upgrade to Tab completion, designed to predict the next change your code needs, wherever it lives. 

NES keeps offering the next edit, then the one after that, and then the one after that. You can walk through these changes across all your files, tapping `Tab` and stacking up saved keystrokes (and time!) with every acceptance.
...
Other tools have versions of this idea, but models have evolved a lot, and so has our understanding of how people actually write code. So we bring you our own take on Pochi’s NES: a bit different, a bit more opinionated.  This is the first post in our "Next Edit Suggestion" series, where we’ll share how we trained, evaluated, and optimized our internal Next Edit model in Pochi.
