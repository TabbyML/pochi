...
Now, we’ll work towards how Pochi can just go from commenting to actually driving workflows. We’ll make Pochi review a PR and identify if it needs changes. If yes, we stop the workflow and address the user to implement the feedback and if everything looks good, we’ll let the pipeline continue only if Pochi approves. 
