...
Throughout the tutorial, we’ll tailor each step that builds on the last, so you’ll see Pochi evolve from a simple reviewer into a full-fledged agent that embeds intelligence inside your development workflow.
...
For Step 1, since Pochi will be reviewing our code, we’ll create a small full-stack Node.js + TypeScript + React snippet with a few intentional mistakes (bad naming, missing error handling, inconsistent types). This way, Pochi’s review will surface meaningful feedback.
...
## Step 2: Automate Reviews Based on Events
...
Based on what we’ve seen so far, the most obvious way would be to add another workflow and test by opening a PR. The event of opening the PR should trigger the workflow since we’re doing event based triggers and we don’t need to type `/pochi` for this use case. 
...
In production repositories, teams often split workflows. For example: `deploy.yml` for deployments, `ci.yml` for tests, and `pochi.yml` for the AI assistant engine. This helps to tweak automation triggers without touching the core runner. Further this also helps with modularity. Suppose later you want two flavours of auto-comments (e.g one for PRs and one for weekly cross). Having them as separate workflows means you can enable / disable independently. 

That being said, you can always merge them into your existing `pochi.yml` file. However, combining everything into one file can become messy. Since GitHub Actions doesn’t support dynamic inline comment bodies well, the two-file approach is cleaner and aligns better with common team practices.

### Custom GitHub Tokens (PAT)

When you create the new `.github/workflows/pochi-auto-comments.yml` workflow, the bot will automatically post a `/pochi summarize this P`R comment on your pull request. However, Pochi won’t respond to it. This is because GitHub has built-in protections: comments made by `github-actions[bot]` (the default Actions identity) do not trigger new workflows. This is done to prevent infinite workflow loops. 

The solution is to use Personal Access Token (PAT) so the auto-comments come from your account instead of `github-actions[bot]`, mimicking as if your commented yourself like we did in Step 1. GitHub treats these PAT-authored comments as real user events which trigger workflows normally.
...
The `pochi.yml` workflow (from Step 1) still listens for `/pochi` comments. Now, `pochi-auto-comments.yml` automatically posts those comments in response to events. To test this out, we’ll make two tiny changes so the PR summary looks legit.
...
## Step 3: Integrations into CI/CD

Now, we’ll work towards how Pochi can just go from commenting to actually driving workflows. We’ll make Pochi review a PR, identify if it needs changes, if yes, we stop the workflow and address the user to implement the feedback and if everything looks good, we’ll let the pipeline continue only if Pochi approves. 
...
**Note:** You’ll temporarily need to comment out your `pochi-auto-comments.yml` file while doing Step 3. Otherwise, both workflows may run simultaneously and interfere with each other.
...
To enhance the workflow further, we’ll show how to add integration tools like Slack notifications to your deployment process. We've demonstrated how Pochi can review your PR, act as a gatekeeper based on the output, and then allow the rest of your pipeline to continue. And now, we’ll also show you how to chain in extra jobs like notifications on top of tests and deploys. 


To showcase this, we'll do Slack notifications. Since this is just GitHub Actions, you can integrate with any API-based tool your team uses, like Linear, Jira, Datadog, PagerDuty, or others. Pochi becomes the gatekeeper, and the rest of the pipeline is up to you. 


### Slack Setup

To add Slack to your pipelines, you’d first need a Slack workspace where you have admin / developer access. Next, you’ll need to create a Slack Incoming Webhook. To create one, go to Slack API Apps  and Create New App. 

- Choose "From scratch" and give it a name (e.g., PochiBot) and select your workspace.
- In the app, enable Incoming Webhooks and add a new webhook to your channel.
- Copy the Webhook URL (it will look like `https://hooks.slack.com/services/T000/B000/XXXX`).
...
>
  <iframe
    src="https://drive.google.com/file/d/1zi32sY54ZDEtk0o37_jcmOR7klLmP46J/preview"
    style={{
      position: "absolute",
      top: 0,
      left: 0,
...
Next, store it in your repository as a GitHub Secret, the same way we stored the PAT token. Go to Repo → Settings → Secrets → Actions → New Secret, and this time name it `SLACK_WEBHOOK_URL`. Paste the webhook URL as the value and save it. 

Finally, add the `notify_slack` job in your `pochi.yml` file. Here’s the updated file:
...
jobs:
  ...
  notify_slack:
    needs: [pochi_review, run_tests, deploy]
    runs-on: ubuntu-latest
    steps:
      - name: Send Slack Notification
        ...
        with:
          payload: |
            {
              "text": "📢 *Pochi Review Result* for PR #${{ github.event.issue.number }}\n*Decision:* ${{ needs.pochi_review.outputs.decision }}\n*Approved:* ${{ needs.pochi_review.outputs.approved }}\n*Repo:* ${{ github.repository }}"
            }
        env:
          SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}

  # Fail workflow run if rejected
  reject_status:
    needs: pochi_review
    if: needs.pochi_review.outputs.approved != 'true'
    runs-on: ubuntu-latest
...
Now you can run the workflow by commenting in the PR: 

```bash
/pochi please review this PR. At the end, on the last line, please output either ✅ APPROVED or ❌ CHANGES_REQUIRED
```

When you visit the workflow run, you’ll see that a new `notify_slack` job was added. On successful run, you get a notification in your Slack workspace. 
...
>
  <iframe
    src="https://drive.google.com/file/d/1qNDDO30UqzweGuZwhIKxubDRdBSx99Ea/preview"
    style={{
      position: "absolute",
      top: 0,
      left: 0,
...
## Step 4: Enforce Coding Rules

Next, let’s look at team-based use cases. In large organisations, developers often have to follow team’s specific coding rules instead of just generic best practices. You can take the learnings from this tutorial so far and create  `.md` file that holds your team coding standard / review ruleset.
...
## Step 6: Enterprise Scaling - Power Up Your Pochi
...
We saw in Step 2, how we created a custom GitHub token (PAT) to mimic user-authored comments. This allows us to maintain GitHub’s built-in protections and avoid infinite workflow loops. This is how enterprises in production codebases grant Pochi extra permissions safely.
...
We started this tutorial with a basic GitHub Actions workflow where Pochi reviewed pull requests on command. Step by step, we extended it into something more powerful that could gatekeeper PRs, enforce custom rules and and handling routine tasks in the background. 
