...
The solution is to use Personal Access Token (PAT) so the auto-comments come from your account instead of `github-actions[bot]`, mimicking as if you commented yourself like we did in Step 1. GitHub treats these PAT-authored comments as real user events which trigger workflows normally.
...
Now, we’ll work towards how Pochi can go from just commenting to actually driving workflows. We’ll make Pochi review a PR and identify if it needs changes. If yes, we stop the workflow and address the user to implement the feedback and if everything looks good, we’ll let the pipeline continue after Pochi has approved. 
...
We saw in Step 2, how we created a custom GitHub token (PAT) to mimic user-authored comments, allowing us to maintain GitHub’s built-in protections and avoid infinite workflow loops. This is how enterprises in production codebases grant Pochi extra permissions safely.
...
We started this tutorial with a basic GitHub Actions workflow where Pochi reviewed pull requests on command. Step by step, we extended it into something more powerful that could gatekeeper PRs, enforce custom rules and handle routine tasks in the background. 
