...
describe("match-slash-command", () => {
  ...
  describe("replaceSlashCommandReferences", () => {
    ...
    it("should replace agent references with content", async () => {
      ...
      expect(result).toBe(
        'Please use <custom-agent id="test-agent" path=".pochi/agents/test-agent.md"/> for this task'
      );
    });
    ...
    it("should handle multiple slash command references", async () => {
      ...
      expect(result).toBe(
        'Use <workflow id="test-workflow" path=".pochi/workflows/test-workflow.md">This is a test workflow</workflow> and then <custom-agent id="test-agent" path=".pochi/agents/test-agent.md"/>',
      );
    });
  });
});
