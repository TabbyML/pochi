...
export const newTask =
  ...
  async ({ description, prompt, _meta, _transient }) => {
    if (!description || !prompt) {
      throw new Error("Description and prompt are required for creating a new task.");
    }
    ...
    if (_transient?.task) {
      ...
      return {
        result: (() => {
          ...
          const completionPart = message.parts?.find(part => part.type === "tool-attemptCompletion") as any;
          return completionPart?.input?.result || "Task completed";
        })()
      };
    }
        ...
        task: async (_, task) => {
          try {
            ...
            if (lastMessage?.role === "assistant") {
              ...
              if (completionPart?.input?.result) {
                result = completionPart.input.result;
              }
            } else {
